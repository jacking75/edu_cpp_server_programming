...
#include "UniqueSeqNumGenerator.h"
...
struct AsyncSocketDesc
{
	ULONG_PTR		assockUid;
	LARGE_INTEGER	tick;

	AsyncSocketDesc() : assockUid(), tick()
	{
	}

	AsyncSocketDesc(ULONG_PTR assockUid, LARGE_INTEGER& tick) : assockUid(assockUid), tick(tick)
	{
	}

	AsyncSocketDesc(ULONG_PTR assockUid, LARGE_INTEGER* atick) : assockUid(assockUid), tick()
	{
		if(atick)
		{
			tick = *atick;
...
struct AsyncSocketDescEx : public AsyncSocketDesc
{
	INetworkSender*	psender;

	AsyncSocketDescEx() : psender()
	{
	}

	AsyncSocketDescEx(INetworkSender* psender) : psender(psender)
	{
	}
};
...
{
private:
	static AsyncIOSocket*		sAsynchSocketPrototype;

protected:
	static UniqueSeqNumGenerator	assockuidGen;
	static CriticalSectionLockWrapper	assockuidGenLock;

protected:
	SOCKET			sockID;
	AsyncSocketDescEx	assockDesc;
	LONG			referCount;
	int				closed;

public:

	static AsyncIOSocket* GetASPrototype()
	{
		Synchronized sync(&assockuidGenLock);
		if(NULL == sAsynchSocketPrototype)
		{
			sAsynchSocketPrototype = new AsyncIOSocket();
		}
...
public:
	AsyncIOSocket() : sockID(INVALID_SOCKET), assockDesc(), referCount(0), closed(0)
	{
	...
	virtual ~AsyncIOSocket()
	{
		//assockuid반환은 asio를 통해서 한다.
	}

	virtual AsyncIOSocket* clone()
	{
		// 그냥 같은 타입의 인스턴스를 하나 생성시킨다.
		return new AsyncIOSocket();
	}
	...
	void getASSOCKDESCEX(AsyncSocketDescEx& output)
	{
		output = assockDesc;
	}
...
public:
	...
	//length: 길이
	//data: 실제 내용
	virtual DWORD postingSend(AsyncSocketDesc& sockdesc, size_t length, char* data) = 0;

	//연결해제 요청
	//uniqueId: 소켓관리번호, 연결통보시 넘겨준 관리번호
	virtual DWORD disconnectSocket(ULONG_PTR uniqueId, LARGE_INTEGER* tick) = 0;
	virtual DWORD disconnectSocket(AsyncSocketDesc& sockdesc) = 0;
	...
	virtual DWORD connectSocket(INT32 reqeusterID, AsyncIOSocket* prototype, char* ip, u_short port ) = 0;
	...
	virtual DWORD registerSocket(SOCKET sockid, AsyncIOSocket* prototype, SOCKADDR_IN& addrin) = 0;
...
public:
	...
	virtual void notifyRegisterSocket(AsyncSocketDescEx& sockdesc, SOCKADDR_IN& ip) = 0;

	//소켓 연결해제 통보
	//socketUniqueId: 연결해제된 소켓의 관리번호, 연결통보시 넘겨준 관리번호
	virtual void notifyReleaseSocket(AsyncSocketDescEx& sockdesc) = 0;

	//수신된 메시지내용 통보
	//socketUniqueId: 수신된 소켓의 관리번호, 연결통보시 넘겨준 관리번호
	//length: 길이
	//data: 실제 내용
	virtual void notifyMessage(AsyncSocketDescEx& sockdesc, size_t length, char* data) = 0;
	...
	virtual void notifyConnectingResult(INT32 requestID, AsyncSocketDescEx& sockdesc, DWORD error) = 0;
};
