...
Acceptor::Acceptor(INetworkSender* theAsynchEventManager, AsyncIOSocket* prototype, const char* address, u_short port) :
m_prototype(prototype), theAsynchEventManager(theAsynchEventManager)
{
	...
	if (socketValue == INVALID_SOCKET) {
		throw new AsyncIOException(GetLastError(), "ListenSocket dont create");
	}
    ...
    {        
		...
		if( 0 != error )
		{
			...
			throw new AsyncIOException(error, estr);
		}
    }
