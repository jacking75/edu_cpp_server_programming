...
AsynchIOService::AsynchIOService(INetworkReceiver* receiver, size_t ioMaxSize, DWORD conThread, char* aname) : receiver(receiver), ioMaxSize(ioMaxSize), conThread(conThread), workers(), iocpHandle(INVALID_HANDLE_VALUE), theASSocks(), entireSynch(), ioFrameSynch(), ioFramePools(), logSynch()
{
	...
	if(NULL == receiver)
		throw new AsyncIOException(WSAEINVAL, "receiver null");

	if(1 > conThread)
		throw new AsyncIOException(WSAEINVAL, "conThread must over than 0");

	if(cIOMINSIZE > ioMaxSize)
		throw new AsyncIOException(WSAEMSGSIZE, "ioMaxSize too small than cIOMINSIZE");

	iocpHandle = ::CreateIoCompletionPort(INVALID_HANDLE_VALUE, NULL, (ULONG_PTR)0, conThread);

	if(INVALID_HANDLE_VALUE == iocpHandle)
		throw new AsyncIOException(GetLastError(), "iocp creation fail");
...
DWORD AsynchIOService::postingRead(AsyncIOSocket* socket, OverlappedOperation* op)
{
...
DWORD AsynchIOService::postingSend(AsyncSocketDesc& sockdesc, size_t length, char* data)
{
...
void AsynchIOService::releaseSocket(AsyncIOSocket* socket, DWORD why)
{
	Synchronized es(&entireSynch);

	AsyncSocketDescEx closedDesc;	
...
DWORD AsynchIOService::disconnectSocket(ULONG_PTR uniqueId, LARGE_INTEGER* tick)
{
	...
	AsyncSocketDesc desc;
	desc.assockUid = uniqueId;
	desc.tick = *tick;	
	return disconnectSocket(desc);
}

DWORD AsynchIOService::disconnectSocket(AsyncSocketDesc& sockdesc)
{		
	Synchronized es(&entireSynch);
...
DWORD AsynchIOService::connectSocket(INT32 reqeusterID, AsyncIOSocket* prototype, char* ip, u_short port)
{
	//synch along entire method
	Synchronized es(&entireSynch);
	...
	// 소켓관리객체를 생성
	AsyncIOSocket* socket = static_cast<AsyncIOSocket*>(prototype->clone());	
...
{
	AsyncIOSocket::releaseASSOCKUID(socketUniqueId);
	return 0;
}

DWORD AsynchIOService::registerSocket(SOCKET sockid, AsyncIOSocket* prototype, SOCKADDR_IN& addrin)
{
	...
	AsyncSocketDescEx desc;
	{
		...
		AsyncIOSocket* as = static_cast<AsyncIOSocket*>(prototype->clone());
		if (NULL == as) {
			return WSAENOBUFS;
		}
