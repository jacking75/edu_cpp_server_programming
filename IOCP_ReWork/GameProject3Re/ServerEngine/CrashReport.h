...
namespace hbServerEngine
{
    ...
    inline LPTOP_LEVEL_EXCEPTION_FILTER g_preFilter;

    inline std::string g_AppName;

    inline _invalid_parameter_handler g_preInvalidParamHandler;

    inline _purecall_handler g_prePureHandler;
    ...
    long __stdcall CrashCallBack(_EXCEPTION_POINTERS* pExInfo)
    {
        ...
        wchar_t  tem2[256] = {0, };
        wsprintf(tem2, L"%S", tem);

        HANDLE hFile = ::CreateFile(tem2, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
