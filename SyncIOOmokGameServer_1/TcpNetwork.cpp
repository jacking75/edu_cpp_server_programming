...
namespace NServerNetLib
{
    ...
    NetError TcpNetwork::SendData(const int sessionIndex, const short packetId, const short bodySize, const char* pMsg)
    {
        ...
        result.Vlaue = send(fd, session.pSendBuffer, totalSize, 0);
...
namespace NServerNetLib
{
    ...
    void TcpNetwork::SendProcess()
    {
        while (mIsRunning)
        {
            ...
            FlushSendBuff(packet.value().SessionIndex);
        }
    }

    void TcpNetwork::FlushSendBuff(const int sessionIndex)
    {
        auto& session = m_ClientSessionPool[sessionIndex];
        auto fd = static_cast<SOCKET>(session.SocketFD);
