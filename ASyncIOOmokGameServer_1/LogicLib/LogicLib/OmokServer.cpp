...
namespace OmokServerLib
{
	...
	ERROR_CODE OmokServer::Init()
	{
		...
		auto sendPacketFunc = [&](const int sessionIndex, const short bodySize, char* pMsg)
		{
			m_pNetwork->SendMsg(sessionIndex, bodySize, pMsg);
		};
	...
	{	
		while (m_IsRun)
		{
			...
			auto systemPacketInfo = m_pNetwork->GetSystemPacketData();

			if (systemPacketInfo.has_value() == true)
			{
				m_pPacketProc->Process(systemPacketInfo.value());
			}
			
			auto packetInfo = m_pNetwork->GetPacketData();

			if (packetInfo.has_value() == true)
			{
