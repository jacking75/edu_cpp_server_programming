...
struct acl_avl_node {
	struct acl_avl_node *avl_child[2];	/* left/right children */
	struct acl_avl_node *avl_parent;	/* this node's parent */
...
struct acl_avl_node {
	struct acl_avl_node *avl_child[2];	/* left/right children nodes */
 ...
 */
#define	AVL_XPARENT(n)		((struct acl_avl_node *)((n)->avl_pcb & ~7))
...
#define	AVL_DATA2NODE(d, o)	((struct acl_avl_node *)((uintptr_t)(d) + (o)))

/*
 * macros used to create/access an acl_avl_index_t
 */
#define	AVL_INDEX2NODE(x)	((acl_avl_node_t *)((x) & ~1))
#define	AVL_INDEX2CHILD(x)	((x) & 1)
#define	AVL_MKINDEX(n, c)	((acl_avl_index_t)(n) | (c))


/*
 * The tree structure. The fields avl_root, avl_compar, and avl_offset come
 * first since they are needed for acl_avl_find().  We want them to fit into
 * a single 64 byte cache line to make acl_avl_find() as fast as possible.
 */
struct acl_avl_tree {
	struct acl_avl_node *avl_root;	/* root node in tree */
 ...
 */
ACL_API void *acl_avl_walk(struct acl_avl_tree *, void *, int);
