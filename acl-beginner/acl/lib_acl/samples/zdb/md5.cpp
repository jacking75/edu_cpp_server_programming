...
const char* MD5Key(const char *s, size_t s_len, const char *k, size_t k_len, void* buf, size_t b_len)
{
	...
	MD5Init(&context);
	if (k != NULL && k_len > 0)
		MD5Update(&context, (const unsigned char *) k, (unsigned int) k_len);
	MD5Update(&context, (const unsigned char*) s, (unsigned int) s_len);
...
const char* MD5String(const char *s, const char *k, size_t k_len, char* buf, size_t size)
{
	...
	if (k != NULL && k_len > 0)
		MD5Update(&context, (const unsigned char *) k, (unsigned int) k_len);
	MD5Update(&context, (const unsigned char*) s, (unsigned int) len);
	MD5Final(digest, &context);
