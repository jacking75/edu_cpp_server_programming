...
{
	ACL_XML3_ATTR *attr = (ACL_XML3_ATTR*)
		acl_dbuf_pool_calloc(node->xml->dbuf, sizeof(ACL_XML3_ATTR));
...
static ACL_XML3_NODE *node_iter_head(ACL_ITER *it, ACL_XML3_NODE *node)
{
	ACL_RING *ring_ptr;
	...
	it->ptr = acl_ring_to_appl(ring_ptr, ACL_XML3_NODE, node);
	it->data = it->ptr;

	return it->ptr;
}

static ACL_XML3_NODE *node_iter_next(ACL_ITER *it, ACL_XML3_NODE *node)
{
	ACL_RING *ring_ptr;
	struct ACL_XML3_NODE *child = (struct ACL_XML3_NODE*) it->data;
...
static ACL_XML3_NODE *node_iter_tail(ACL_ITER *it, ACL_XML3_NODE *node)
{
	ACL_RING *ring_ptr;
...
static ACL_XML3_NODE *node_iter_prev(ACL_ITER *it, ACL_XML3_NODE *node)
{
	ACL_RING *ring_ptr;
	struct ACL_XML3_NODE *child = (struct ACL_XML3_NODE*) it->data;
...
{
	ACL_XML3_NODE *node = (ACL_XML3_NODE*)
		acl_dbuf_pool_calloc(xml->dbuf, sizeof(ACL_XML3_NODE));

	acl_ring_init(&node->children);
	acl_ring_init(&node->node);

	node->xml       = xml;
	node->status    = ACL_XML3_S_NXT;
	node->ltag      = xml->addr;
	node->rtag      = xml->addr;
	node->ltag_size = 0;
	node->rtag_size = 0;
...
{
	ACL_RING *next;
	ACL_XML3_NODE *node_next;
	int   n = 1;

	while ((next = acl_ring_pop_head(&node->children)) != NULL) {
		node_next = acl_ring_to_appl(next, ACL_XML3_NODE, node);
		n += acl_xml3_node_delete(node_next);
	}
...
ACL_XML3_NODE *acl_xml3_node_next(ACL_XML3_NODE *node)
{
	...
	return acl_ring_to_appl(ring_ptr, ACL_XML3_NODE, node);
}
...
static ACL_XML3_NODE *xml_iter_head(ACL_ITER *it, ACL_XML3 *xml)
{
	ACL_RING *ring_ptr;
...
static ACL_XML3_NODE *xml_iter_next(ACL_ITER *it, ACL_XML3 *xml)
{
	ACL_RING *ring_ptr;
	struct ACL_XML3_NODE *node, *parent;

	node = (struct ACL_XML3_NODE*) it->data;
	...
	parent = acl_xml3_node_parent(node);
...
static ACL_XML3_NODE *xml_iter_tail(ACL_ITER *it, ACL_XML3 *xml)
{
	ACL_RING *ring_ptr;
...
static ACL_XML3_NODE *xml_iter_prev(ACL_ITER *it, ACL_XML3 *xml)
{
	ACL_RING *ring_ptr;
	struct ACL_XML3_NODE *node, *parent;
...
{
	if (on)
		xml->flag |= ACL_XML3_FLAG_MULTI_ROOT;
	else
		xml->flag &= ~ACL_XML3_FLAG_MULTI_ROOT;
}
...
ACL_XML3 *acl_xml3_dbuf_alloc(ACL_DBUF_POOL *dbuf)
{
	...
	if (dbuf == NULL) {
		...
		xml = (ACL_XML3*) acl_dbuf_pool_calloc(dbuf, sizeof(ACL_XML3));
		xml->dbuf_inner = dbuf;
	} else {
		xml = (ACL_XML3*) acl_dbuf_pool_calloc(dbuf, sizeof(ACL_XML3));
		xml->dbuf_inner = NULL;
	...
	xml->root      = acl_xml3_node_alloc(xml);
	xml->node_cnt  = 1;
...
int acl_xml3_is_closure(ACL_XML3 *xml)
{
	...
	node = acl_ring_to_appl(ring_ptr, ACL_XML3_NODE, node);

	if ((node->flag & ACL_XML3_F_SELF_CL)) {
		/* 说明该节点是自闭合节点 */
		return 1;
	}

	if (node->status == ACL_XML3_S_RGT) {
		return 1;
	}
...
int acl_xml3_is_complete(ACL_XML3 *xml, const char *tag)
{
	...
	if ((last_node->flag & ACL_XML3_F_SELF_CL))
		/* 说明该节点是自闭合节点 */
		return 1;

	if (last_node->status != ACL_XML3_S_RGT)
		/* 说明最后一个一级子节点还未处理完毕 */
		return 0;
